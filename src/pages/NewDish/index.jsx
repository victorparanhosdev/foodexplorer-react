...
export function NewDish() {
    ...
    async function handleNewDish(event) {
        ...
        if (!price) {
            return alert(`Coloque um Preço para a ${transformarParaSingular(category)} e use "." para separar os centavos`)
        }
        if (!price) {
            return alert(`Coloque um Preço para a ${transformarParaSingular(category)} e use "." para separar os centavos`)
        }
                ...
                <form action="" className="edit-content">
                    <div>
                        ...
                        <div>
                            ...
                            }} type="text" placeholder="Ex.: Salada Ceasar" />
                        </div>

                        <div>
                            <span>Categoria</span>
                            <select onChange={e => setCategory(e.target.value)} defaultValue="Refeições" name="" id="">
                        ...
                        <div>
                            ...
                            <div>
                                {ingredients && ingredients.map((tag, index) =>
                                    <NewTag onClick={() => handleClickRemoveIngredients(tag)} key={String(index)} value={tag} />
                                )}
                                <NewTag value={newingredients} onChange={e => setNewIngredients(e.target.value)} onClick={handleClickNewIngredients} isnew="true" placeholder="Adicionar" />
                            </div>

                        </div>
                        <div>
                            <span>Preço</span>
                            <input onChange={e => setPrice(e.target.value)} type="number" placeholder="R$00.00" />
                        </div>
                    ...
                    <div>
                        <span>Descrição</span>
                        <textarea onChange={e => setDescription(e.target.value)} placeholder="Fale brevemente sobre o prato, seus ingredientes e composição" name="" id="" cols="30" rows="10"></textarea>

                    </div>
                    <button onClick={e => handleNewDish(e)} type="submit">Salvar Alterações</button>
                </form>
